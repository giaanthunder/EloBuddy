...
namespace xRP_Ashe
{
    internal class Program
    {
        ...
        public static void OnLoad_Complete(EventArgs args)
        {
            ...
            AsheMenu = MainMenu.AddMenu("Ashe", "sbtwashe");
            AsheMenu.AddGroupLabel("Ashe");
            AsheMenu.AddSeparator();
            AsheMenu.AddLabel("Made by: giaanthunder");
        ...
        private static void Combo()
        {
            ...
            if (W.IsReady() && useW)
            {
                ...
                {
					var hitchance = predi.GetHitChance(SpellSlot.W, 1200, SkillShotType.Linear, 0, 2000, 50,Me.ServerPosition,targetw);
		            if (hitchance >= HitChance.Medium)
		            {
		                predi.CastPredictedSpell(SpellSlot.W, 1200, SkillShotType.Linear, 0, 2000, 50,targetw, true, false);
		            }
                }
            }
            ...
            if (R.IsReady() && useR)
            {
                var targetr = TargetSelector.GetTarget(R.Range, DamageType.Physical);
                {
                    if (targetr.IsValidTarget(R.Range))
                    {
                        ...
                        {
							var hitchance = predi.GetHitChance(SpellSlot.R, 3000, SkillShotType.Linear, 250, 1600, 130,Me.ServerPosition,targetr);
				            if (hitchance >= HitChance.Medium)
				            {
				                predi.CastPredictedSpell(SpellSlot.R, 3000, SkillShotType.Linear, 250, 1600, 130,targetr, false, false);
				            }
                        }
                    }
                }
