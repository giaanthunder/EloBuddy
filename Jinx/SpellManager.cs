...
//                }
				var hitchance = predi.GetHitChance(SpellSlot.W, 1450, SkillShotType.Linear, 600, 3300, 60,Player.Instance.ServerPosition,target);
	            if (hitchance >= MenuManager.PredW())
	            {
	                predi.CastPredictedSpell(SpellSlot.W, 1450, SkillShotType.Linear, 600, 3300, 60,target, true, false);
	            }
            }
        }
...
//                }
				var hitchance = predi.GetHitChance(SpellSlot.E, 900, SkillShotType.Circular, 1200, 0, 120,Player.Instance.ServerPosition,target);
	            if (hitchance >= MenuManager.PredE())
	            {
	                predi.CastPredictedSpell(SpellSlot.E, 900, SkillShotType.Circular, 1200, 0, 120,target, false, false);
	            }
            }
        }
        internal static void CastESlowed(Obj_AI_Base target)
        {
            if (E.IsReady() && Util.MyHero.IsInRange(target, E.Range + E.Width + target.BoundingRadius / 2f))
            {
                ...
                {
                    var pred = E.GetPrediction(target);
                    if (pred.HitChance >= MenuManager.PredE())
                    {
                        E.CastDelay = 600;
