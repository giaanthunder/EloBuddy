...
using Lux.DMGHandler;
using myPrediction;
using pred=myPrediction.myPrediction;

using Settings = Lux.Config.Modes.Misc;
...
//                        E.Cast(E.GetPrediction(targetE).CastPosition);
            ...
            if (Q.IsReady() && target.IsValidTarget(Q.Range) && Config.Modes.Harass.UseQ)
            {
            ...
            if (E.IsReady() && target.IsValidTarget(E.Range) && Config.Modes.Harass.UseE)
            {
