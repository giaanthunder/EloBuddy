        ...
        /* public static void FleeMode()
         {
             ...
             {
                 var point = Flee.GetPoint();
                 var monster = EntityManager.MinionsAndMonsters.GetJungleMonsters(Player.Instance.ServerPosition,
                                     Spells.W2p.Range)
                                     .Where(t => t.IsValidTarget() && JungleMobsList.Contains(t.BaseSkinName))
                                     .OrderByDescending(t => t.MaxHealth)
                                     .FirstOrDefault();
                 var pred = Spells.W2p.GetPrediction(monster);
                 if (Player.Instance.Distance(point[0]) <= 750)
                 {
                     if (!Event.Humanform() && Spells.W2p.IsReady(1) && Spells.R.IsReady() && Event.IsReady(Event.CD["Javelintoss"]))
                     {
                         Spells.R.Cast();
                         if (Event.Humanform() && Spells.Q.IsReady() && Spells.R.IsReady(300) && Event.IsReady(Event.CD["Pounce"], 0.4f))
                         {
                             Spells.Q.Cast(point[0]);
                             Core.DelayAction(() => Spells.R.Cast(), 600);
                             if (monster == null) return;
                             {
                                 Spells.W2p.Cast(pred.CastPosition);
                             }
                         }
                         if (Event.Humanform() && Spells.W.IsReady() && Spells.R.IsReady(600) && Event.IsReady(Event.CD["Pounce"], 0.4f))
                         {
                             Spells.W.Cast(point[0]);
                             Core.DelayAction(() => Spells.R.Cast(), 600);
                             if (monster == null) return;
                             {
                                 Spells.W2p.Cast(pred.CastPosition);
         ...
         I THINK IT NEED REWORK */

        /*public static void InitPoints()
        {
            //frog (west)
            Points.Add(new[] { new Vector3(2111, 8450, 52) });

            //blue (west)
            Points.Add(new[] { new Vector3(3638, 7839, 54) });

            //wolf (west)
            Points.Add(new[] { new Vector3(3731, 6694, 52) });

            //bird (south)
            Points.Add(new[] { new Vector3(6914, 5608, 58) });

            //red (south)
            Points.Add(new[] { new Vector3(7919, 3881, 52) });

            //krug (south)
            Points.Add(new[] { new Vector3(8323, 2754, 51) });

            //frog (east)
            Points.Add(new[] { new Vector3(12704, 6444, 52) });

            //blue (east)
            Points.Add(new[] { new Vector3(11068, 6790, 52) });

            //wolf (east)
            Points.Add(new[] { new Vector3(10808, 8387, 63) });

            //bird (nouth)
            Points.Add(new[] { new Vector3(7654, 9610, 52) });

            //red (nouth)
            Points.Add(new[] { new Vector3(6917, 11003, 56) });

            //krug (nouth)
            Points.Add(new[] { new Vector3(6547, 12156, 56) });
