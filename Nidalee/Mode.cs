...
//                        if (target.IsValidTarget() && Qgp.HitChancePercent >= Qhitchance)
//                        {
//                            Spells.Q.Cast(Qgp.UnitPosition);
//                        }
						var hitchance = predi.GetHitChance(SpellSlot.Q, 1500, SkillShotType.Linear, 250, 1300, 55,Player.Instance.ServerPosition,target);
			            if (hitchance >= Config.PredQ1())
			            {
			                predi.CastPredictedSpell(SpellSlot.Q, 1500, SkillShotType.Linear, 250, 1300, 55,target, true, false);
			            }
                    }
                }
...
//                        }
                        var hitchance = predi.GetHitChance(SpellSlot.W, 900, SkillShotType.Circular, 750, int.MaxValue, 40,Player.Instance.ServerPosition,target);
			            if (hitchance >= Config.PredW1())
			            {
			                predi.CastPredictedSpell(SpellSlot.W, 900, SkillShotType.Circular, 750, int.MaxValue, 40,target, false, false);
			            }
                    }
                }
        ...
        public static void Harass()
        {
            if (Event.Humanform())
            {
                ...
                {
                    var target = TargetSelector.GetTarget(Spells.Q.Range, DamageType.Magical);
                    if (target != null && Orbwalker.CanMove)
                    {
//                        var Qgp = Spells.W.GetPrediction(target);
//                        if (target != null && target.IsValidTarget() && Qgp.HitChancePercent >= Qhitchance)
//                        {
...
//                        }
        ...
        public static void LaneClear()
        {
            ...
            if (!Event.Humanform() && Orbwalker.CanMove)
            {
        ...
        public static void JungleClear()
        {
            ...
            if (Config.RMenu["Rformjc"].Cast<CheckBox>().CurrentValue && Orbwalker.CanMove)
            {
                if (Event.Humanform())
                {
                    if (!Spells.Q.IsReady() && !Config.WMenu["Wcb"].Cast<CheckBox>().CurrentValue && Spells.R.IsReady())
