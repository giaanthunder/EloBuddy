...
//                        return;
//                    }
					var hc = pred.GetHitChance(SpellSlot.W, 900, SkillShotType.Circular, .650f, 0, 250,Player.Instance.Position,selectedTarget);
		            if (hc >= HitChance.High)
		            {
		                pred.CastPredictedSpell(SpellSlot.W, 900, SkillShotType.Circular, .650f, 0, 250,selectedTarget, false, false);
		            }
                }
            }
...
//                        return;
//                    }
					var hc = pred.GetHitChance(SpellSlot.Q, 1050, SkillShotType.Linear, .25f, 1600, 70,Player.Instance.Position,selectedTarget);
		            if (hc >= HitChance.High)
		            {
		                pred.CastPredictedSpell(SpellSlot.Q, 1050, SkillShotType.Linear, .25f, 1600, 70,selectedTarget, true, false);
		            }
                }
            }
        }
