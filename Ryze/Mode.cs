...
using SharpDX;
using myPrediction;
using predd=myPrediction.myPrediction;
...
//                                    Spells.Q.Cast(pred.CastPosition);
									predd.CastPredictedSpell(SpellSlot.Q, 1000, SkillShotType.Linear, .250f, 1700, 100,target, true, false);
                                }
                                if (target != null && target.IsValid() && target.HasBuff("RyzeE"))
                                {
                                    var pred = Spells.Q.GetPrediction(target);
...
//                                                Spells.Q.Cast(CollisionObjectE.First());
                                                predd.CastPredictedSpell(SpellSlot.Q, 1000, SkillShotType.Linear, .250f, 1700, 100,CollisionObjectE.First(), true, false);
                                            }
                                        }
                                        else
                                        {
...
//                            var pred = Spells.Q.GetPrediction(target);
//                            Spells.Q.Cast(pred.CastPosition);
                            predd.CastPredictedSpell(SpellSlot.Q, 1000, SkillShotType.Linear, .250f, 1700, 100,target, false, false);
                        }
                        if (target != null && target.IsValid() && target.HasBuff("RyzeE"))
                        {
                            var pred = Spells.Q.GetPrediction(target);
